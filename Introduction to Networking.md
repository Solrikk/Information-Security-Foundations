...
`Сетевая топология` представляет собой типичную организацию и `физическое` или `логическое` соединение устройств в сети. Компьютеры являются `хостами`, такими как `клиенты` и `серверы`, которые активно используют сеть. Они также включают `сетевые компоненты`, такие как `коммутаторы`, `мосты` и `маршрутизаторы`, которые мы рассмотрим более подробно в следующих разделах, выполняющие функцию распределения и обеспечивающие возможность логического соединения всех сетевых хостов друг с другом. Сетевая топология определяет используемые компоненты и методы доступа к среде передачи.

`Расположение среды передачи`, используемое для соединения устройств, является физической топологией сети. Для проводных или оптоволоконных сред это относится к плану прокладки кабелей, расположению `узлов` и соединениям между узлами и кабелями. Напротив, `логическая топология` определяет, как сигналы действуют в сетевой среде или как данные будут передаваться по сети от одного устройства к устройствам физического соединения.

---

Мы можем разделить всю область сетевых топологий на три зоны:

#### 1. Соединения

| `Проводные соединения` | `Беспроводные соединения` |
|----------------------|--------------------------|
| Коаксиальные кабели   | Wi-Fi                   |
| Оптоволоконные кабели | Сотовая связь           |
| Витая пара           | Спутниковая связь        |
| и другие             | и другие                 |

---

#### 2. Узлы - Сетевые интерфейсные контроллеры (NICs)

||||
|-----------|-------|---------|--------|
| Повторители | Хабы | Мосты | Коммутаторы |
| Маршрутизаторы/Модемы | Шлюзы | Брандмауэры |  |

Сетевые узлы - это `точки подключения передающей среды` к передатчикам и приемникам электрических, оптических или радиосигналов в среде. Узел может быть подключен к компьютеру, но некоторые типы могут иметь только один микроконтроллер на узле или не иметь программируемого устройства вообще.

---

#### 3. Классификации

Мы можем представить топологию как виртуальную форму или `структуру сети`. Эта форма не обязательно соответствует фактическому физическому расположению устройств в сети. Поэтому эти топологии могут быть как `физическими`, так и `логическими`. Например, компьютеры в `локальной сети` могут располагаться в комнате по кругу, но маловероятно, что они имеют фактическую кольцевую топологию.
...
Самая простая сетевая топология с выделенным соединением между двумя хостами - это топология `точка-точка`. В этой топологии существует прямая и простая физическая связь только между `двумя хостами`. Эти два устройства могут использовать эти соединения для взаимной коммуникации.

Топологии `точка-точка` являются базовой моделью традиционной телефонии и не следует путать с архитектурой `P2P` (`Peer-to-Peer`).

#### Топология точка-точка

![изображение](https://academy.hackthebox.com/storage/modules/34/topo_p2p.png)

---

## Шина

В топологии шины все хосты подключены через среду передачи. Каждый хост имеет доступ к среде передачи и сигналам, которые через неё передаются. Нет центрального сетевого компонента, который контролирует процессы в ней. Средой передачи может быть, например, `коаксиальный кабель`.

Поскольку среда разделяется со всеми остальными, только `один хост может отправлять`, а все остальные могут только получать и оценивать данные, определяя, предназначены ли они для них.

#### Топология шина

![Диаграмма, показывающая соединение между хостами A и B](https://academy.hackthebox.com/storage/modules/34/topo_bus.png)

---

## Звезда

Топология звезды представляет собой сетевой компонент, который поддерживает соединение со всеми хостами. Каждый хост подключен к `центральному сетевому компоненту` через отдельное соединение. Обычно это маршрутизатор, хаб или коммутатор. Они выполняют `функцию пересылки` пакетов данных. Для этого пакеты данных принимаются и пересылаются к месту назначения. Трафик данных на центральном сетевом компоненте может быть очень высоким, так как все данные и соединения проходят через него.
...
Это означает, что один кабель приходит к каждому хосту и один кабель уходит. Кольцевая топология обычно не требует активного сетевого компонента. Управление и доступ к среде передачи регулируются протоколом, которому следуют все станции.
...
Информация передается в заранее определенном направлении передачи. Обычно доступ к среде передачи осуществляется последовательно от станции к станции с использованием поисковой системы от центральной станции или `токена`. Токен — это шаблон битов, который непрерывно проходит через кольцевую сеть в одном направлении, работая по `процессу запроса токена`.
...
Многие узлы принимают решения о соединениях на `физическом` уровне и маршрутизации на `логическом` уровне в ячеистых сетях. Таким образом, ячеистые структуры не имеют фиксированной топологии. Существуют две основные структуры из базовой концепции: `полностью ячеистая` и `частично ячеистая` структура.

Каждый хост подключен к каждому другому хосту в сети в `полностью ячеистой структуре`. Это означает, что хосты связаны друг с другом. Эта техника в основном используется в `глобальных сетях` или `городских сетях` для обеспечения высокой надежности и пропускной способности.

В этой настройке важные сетевые узлы, такие как маршрутизаторы, могут быть объединены в сеть. Если один маршрутизатор выходит из строя, остальные могут продолжать работать без проблем, и сеть может поглотить сбой из-за множества соединений.

Каждый узел полностью ячеистой топологии имеет одинаковые функции маршрутизации и знает соседние узлы, с которыми он может установить связь, близость к сетевому шлюзу и нагрузку трафика.

В `частично ячеистой структуре` конечные точки соединены только одним соединением. В этом типе сетевой топологии определенные узлы подключены ровно к одному другому узлу, а некоторые другие узлы подключены к двум или более другим узлам с помощью соединения типа точка-точка.
...
Топология дерева — это расширенная звездная топология, которую имеют более обширные локальные сети с такой структурой. Это особенно полезно при объединении нескольких топологий. Эта топология часто используется, например, в больших офисных зданиях компаний.

Существуют как логические древовидные структуры в соответствии с `покрывающим деревом`, так и физические. Модульные современные сети, основанные на структурированной кабельной разводке с иерархией концентраторов, также имеют древовидную структуру. Древовидные топологии также используются для `широкополосных сетей` и `городских сетей` (`MAN`).

#### Топология дерево

![Сетевая диаграмма, показывающая сервер, подключенный к двум коммутаторам Ethernet, каждый из которых соединяет хосты A, B, C, D, E и F](https://academy.hackthebox.com/storage/modules/34/topo_tree.png)

---

## Гибридная

Гибридные сети объединяют две или более топологии, так что результирующая сеть не представляет собой стандартную топологию. Например, древовидная сеть может представлять собой гибридную топологию, в которой звездные сети соединены через взаимосвязанные шинные сети. Однако древовидная сеть, связанная с другой древовидной сетью, по-прежнему топологически является древовидной сетью. Гибридная топология всегда создается при взаимном соединении `двух различных` базовых сетевых топологий.
...
Поскольку это создает цепочку соединений, она также известна как конфигурация последовательной цепи, в которой несколько аппаратных компонентов соединены последовательно. Этот тип сети часто встречается в технологии автоматизации (`CAN`).

Последовательная цепь основана на физическом расположении узлов, в отличие от токеновых процедур, которые структурны, но могут быть независимыми от физического расположения. Сигнал отправляется к компоненту и от него через его предыдущие узлы в компьютерную систему.

...
В дополнение к примерам и демонстрациям, представленным в интерактивных разделах, большинство из них заканчиваются упражнениями для проверки этих знаний.

Упражнение обычно имеет сопровождающую `Docker-цель` или `VM-цель(и)`. Цель можно запустить, нажав на `Click here to spawn the target system!` (Нажмите здесь, чтобы запустить целевую систему), после чего будут отображены данные для доступа в формате `http://<ip>:<port>`. Также могут быть предоставлены данные для аутентификации в виде `username` (имя пользователя) и `password` (пароль).

![Раздел с инструкциями для ответа на вопросы и ссылкой для запуска целевой системы, вместе с кнопкой 'Шпаргалка'.](https://academy.hackthebox.com/storage/modules/15/target_example.jpg)

> **Примечание:** Одновременно может быть активна только одна цель для каждого пользователя. Каждая цель имеет таймер, который показывает, сколько времени осталось до ее завершения, но его можно вручную продлить или перезапустить, нажав на соответствующие кнопки.

## Docker-цели

Большинство упражнений, особенно в веб-модулях, используют Docker-цели, которые быстрее запускаются и доступны без дополнительной настройки.

Когда Docker-цель активна, вы можете просто нажать на IP/PORT, чтобы скопировать его, а затем получить доступ к цели (например, в веб-браузере для веб-модулей). Большинство Docker-целей готовы к использованию мгновенно, хотя некоторым может потребоваться до минуты после отображения IP/PORT.

![GIF-демонстрация запуска и копирования адреса цели, а также навигации к цели через браузер.](https://academy.hackthebox.com/storage/modules/15/solving_interactive_exercise.png)

> **Примечание:** Docker-цель можно распознать по отсутствию кнопки VPN, так как для доступа к ней не требуется VPN-подключение.

## VM-цели

Некоторые модули имеют продвинутые требования для своих упражнений, такие как Windows-цель, цель Active Directory (AD) или сетевая среда. Такие модули используют виртуальную машину (VM) в качестве цели, которая может быть запущена как Docker-цель, но для запуска может потребоваться немного больше времени.

Если вы используете свою `Рабочую станцию`, то вы можете начать доступ к VM, как только будет показан её IP. В противном случае, если вы предпочитаете использовать свою собственную машину, то доступ к VM можно получить, подключившись к предоставленному VPN-ключу, который можно скачать, нажав на кнопку `Download VPN Connection File` (Скачать файл подключения VPN).

![Инструкции для ответа на вопросы, IP цели 10.129.43.22, 119 минут осталось, детали RDP и кнопки для Шпаргалки и загрузки VPN.](https://academy.hackthebox.com/storage/modules/15/vm_example.jpg)

> **Примечание:** Чтобы узнать больше о подключении к VPN HTB Academy, ознакомьтесь с [этой статьей помощи](https://help.hackthebox.com/en/articles/9297532-connecting-to-academy-vpn).

## Выполнение упражнения

Некоторые вопросы имеют кнопку `Hint` (Подсказка), которая может указать вам правильное направление, если у вас возникли трудности с ними. Получив ответ или флаг, вы можете ввести его в поле и нажать `Submit` (Отправить), чтобы выполнить упражнение/вопрос.

Правильное решение вопроса вознаградит вас определенным количеством (<i class="fad fa-cube text-success"></i>) кубиков, которые можно собрать для разблокировки других модулей в HTB Academy, как мы обсудим в следующем разделе.

Большинство вопросов необходимы для завершения раздела, но любые вопросы, помеченные как `Optional Exercises` (Дополнительные упражнения), могут быть решены нажатием кнопки `Reveal Answer` (Показать ответ), и раздел может быть завершен без их решения.

> **Примечание:** Вы всегда можете использовать цели упражнений для практики того, что было показано в разделе. Однако решение упражнения не всегда будет соответствовать точным командам, показанным в разделе, так как они будут иметь некоторые вариации для проверки вашего понимания и постепенного развития ваших навыков. Упражнения в более легких модулях будут иметь небольшие отличия от показанной демонстрации, в то время как более сложные модули будут более сложными.

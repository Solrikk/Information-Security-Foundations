...
|----------|----------------|----------------------------|
| **`/`** | **Корневая директория** | Каталог верхнего уровня файловой системы Linux. Содержит все критически важные файлы и каталоги для загрузки и функционирования ОС.<br><br>**🔒 Безопасность:** Права доступа критичны для безопасности системы. Мониторинг изменений помогает выявить компрометации.<br><br>**⚙️ Администрирование:** Регулярная проверка целостности через AIDE/Tripwire, настройка правильных прав доступа. |
| **`/bin`** | **Основные команды** | Важнейшие системные команды и утилиты: `ls`, `cp`, `mv`, `rm`, `cat`, `grep`, `bash`, `chmod`, `chown`, `ps`, `kill`, `mount`, `umount`. Доступны всем пользователям, необходимы в однопользовательском режиме и для базового функционирования системы. Исторически в некоторых дистрибутивах /bin является символической ссылкой на /usr/bin.<br><br>**🔒 Безопасность:** Популярный вектор атак — замена исполняемых файлов вредоносными версиями. Обязательна проверка хэш-сумм через `sha256sum`, использование `rpm -V` или `dpkg --verify`. Важно отслеживать изменения в SUID/SGID битах на исполняемых файлах.<br><br>**⚙️ Администрирование:** Мониторинг через auditd (`auditctl -w /bin -p wa`), использование `chattr +i` для критических файлов, регулярная проверка целостности через AIDE, настройка IDS/HIDS для обнаружения изменений. |
| **`/boot`** | **Файлы загрузки** | Файлы для загрузки ОС: ядро Linux (vmlinuz), initrd/initramfs, конфигурации GRUB, карты системы.<br><br>**🔒 Безопасность:** Модификация позволяет полную компрометацию. Необходимы Secure Boot, подписанные ядра, защита GRUB паролем.<br><br>**⚙️ Администрирование:** Бэкап конфигураций GRUB, мониторинг изменений, LUKS шифрование от физического доступа. |
| **`/dev`** | **Файлы устройств** | Специальные файлы устройств: блочные (диски, USB), символьные (терминалы), псевдо-устройства (/dev/null, /dev/zero, /dev/random).<br><br>**🔒 Безопасность:** Контроль доступа через udev правила. /dev/kmem, /dev/mem критичны для безопасности.<br><br>**⚙️ Администрирование:** Настройка udev правил, мониторинг через dmesg, контроль прав доступа к портам. |
| **`/etc`** | **Конфигурации системы** | Центральное хранилище конфигураций: /etc/passwd (пользователи), /etc/shadow (пароли), /etc/group (группы), /etc/hosts (локальные DNS записи), /etc/resolv.conf (DNS серверы), /etc/fstab (файловые системы), /etc/crontab (задачи планировщика), настройки служб в /etc/systemd/, скрипты инициализации, сетевые конфигурации в /etc/network/ или /etc/sysconfig/network-scripts/.<br><br>**🔒 Безопасность:** /etc/passwd, /etc/shadow, /etc/sudoers — ключевые файлы контроля доступа. PAM конфигурации в /etc/pam.d/, SSH настройки в /etc/ssh/sshd_config, файрвол правила в /etc/iptables/, SSL сертификаты в /etc/ssl/. Важно защищать /etc/sudoers от несанкционированных изменений.<br><br>**⚙️ Администрирование:** Версионирование через Git, etckeeper для автоматического коммита изменений, регулярные бэкапы в /etc/backup/, аудит через auditd (`auditctl -w /etc -p wa`), использование `visudo` для безопасного редактирования sudoers. |
| **`/home`** | **Домашние каталоги** | Личные каталоги пользователей (/home/username/) для документов, настроек, персональных данных.<br><br>**🔒 Безопасность:** SSH ключи в ~/.ssh/, возможные пароли в конфигах. Шифрование через eCryptfs.<br><br>**⚙️ Администрирование:** Дисковые квоты, мониторинг пространства, бэкапы, интеграция с LDAP/AD. |
| **`/lib`** | **Системные библиотеки** | Разделяемые библиотеки (.so) для программ из /bin и /sbin. Включает libc, библиотеки загрузки, модули ядра.<br><br>**🔒 Безопасность:** Подмена библиотек — метод руткитов. Проверка целостности через package manager.<br><br>**⚙️ Администрирование:** ldconfig для обновления кэша, управление модулями ядра, ASLR защита. |
| **`/media`** | **Съемные носители** | Автоматическое монтирование USB, CD/DVD, карт памяти через udisks/automount.<br><br>**🔒 Безопасность:** Вектор атак (BadUSB). Политики автомонтирования, антивирусное сканирование.<br><br>**⚙️ Администрирование:** Udev правила, noexec/nosuid опции, ClamAV сканирование, LUKS шифрование. |
| **`/mnt`** | **Временное монтирование** | Ручное монтирование файловых систем администраторами. Часто создаются подкаталоги (/mnt/usb, /mnt/cdrom).<br><br>**🔒 Безопасность:** Понимание опций безопасности (noexec, nosuid, nodev), контроль прав доступа.<br><br>**⚙️ Администрирование:** fstab для постоянных монтирований, systemd mount units, NFS/CIFS настройка. |
| **`/opt`** | **ПО третьих сторон** | Крупные программные пакеты третьих сторон (/opt/application_name/). Альтернатива /usr/local.<br><br>**🔒 Безопасность:** Возможные уязвимости в стороннем ПО. Проверка подписей, изоляция через контейнеры.<br><br>**⚙️ Администрирование:** Управление версиями, автоматизация через Ansible/Puppet, SELinux/AppArmor политики. |
| **`/root`** | **Каталог root** | Домашний каталог суперпользователя. В корне для доступности при проблемах с /home.<br><br>**🔒 Безопасность:** Полные привилегии. Ограничение SSH входа, использование sudo, 2FA.<br><br>**⚙️ Администрирование:** Версионирование конфигов, логирование действий через auditd, bastion hosts. |
| **`/sbin`** | **Системное администрирование** | Программы для админов: fsck, mkfs, ifconfig, route, системные службы. Доступ только с правами.<br><br>**🔒 Безопасность:** Критичные инструменты. Контроль целостности, ограничение через sudo.<br><br>**⚙️ Администрирование:** iptables, systemctl, crontab, мониторинг сети, LVM/RAID, NTP настройка. |
| **`/tmp`** | **Временные файлы** | Временные файлы системы и приложений. Права 1777 (sticky bit). Очистка при перезагрузке.<br><br>**🔒 Безопасность:** Используется для вредоносных скриптов. Монтирование с noexec, мониторинг активности.<br><br>**⚙️ Администрирование:** tmpfs в RAM, автоочистка через systemd-tmpfiles, отдельный раздел с noexec. |
| **`/usr`** | **Пользовательские программы** | Основные пользовательские программы и данные: /usr/bin (пользовательские команды), /usr/sbin (системные команды), /usr/lib и /usr/lib64 (библиотеки), /usr/share (разделяемые данные, документация, иконки), /usr/local (локально установленное ПО), /usr/src (исходные коды), /usr/include (заголовочные файлы для разработки).<br><br>**🔒 Безопасность:** /usr/local требует особого аудита, так как содержит ПО вне пакетного менеджера. Монтирование /usr в режиме readonly значительно повышает безопасность. Проверка подписей пакетов через GPG, контроль целостности через `rpm -Va` или `debsums`.<br><br>**⚙️ Администрирование:** Управление пакетами через yum/dnf/apt/zypper, проверка целостности установленного ПО, автоматизация патчинга через yum-cron/unattended-upgrades, использование репозиториев с подписями, изоляция /usr/local для стороннего ПО. |
| **`/proc`** | **Виртуальная файловая система** | Интерфейс к ядру Linux. Информация о процессах (/proc/PID/), системные параметры (/proc/sys/), состояние оборудования (/proc/cpuinfo, /proc/meminfo), сетевые подключения (/proc/net/), загруженные модули (/proc/modules).<br><br>**🔒 Безопасность:** Источник информации для реконнесанса атакующих. Некоторые файлы могут раскрывать чувствительную информацию о системе.<br><br>**⚙️ Администрирование:** Мониторинг системы через /proc/stat, /proc/loadavg, настройка ядра через /proc/sys/ (sysctl), отладка процессов. |
| **`/sys`** | **Системная информация** | Современный интерфейс к устройствам и подсистемам ядра. Управление питанием, устройствами, модулями ядра. Заменяет устаревшие интерфейсы /proc для управления оборудованием.<br><br>**🔒 Безопасность:** Контроль энергопотребления, управление USB устройствами, настройка безопасности оборудования.<br><br>**⚙️ Администрирование:** udev использует /sys для управления устройствами, настройка CPU governor, контроль температуры, управление LED индикаторами. |
| **`/var`** | **Переменные данные** | Изменяемые файлы: логи (/var/log - системные логи, веб-серверы, приложения), почта (/var/mail, /var/spool/mail), кэш (/var/cache - пакетные менеджеры, веб-кэш), БД (/var/lib - MySQL, PostgreSQL, системные службы), очереди (/var/spool - печать, cron, at), временные файлы (/var/tmp - сохраняются между перезагрузками), веб-контент (/var/www), FTP (/var/ftp).<br><br>**🔒 Безопасность:** /var/log критичен для форензики и анализа инцидентов. Ротация логов через logrotate предотвращает переполнение диска. Защита логов от модификации через `chattr +a`, удаленная отправка логов на syslog сервер. Мониторинг /var/lib для обнаружения изменений в базах данных.<br><br>**⚙️ Администрирование:** rsyslog/journald для централизованного логирования, logrotate для автоматической ротации, централизованное логирование через ELK Stack (Elasticsearch, Logstash, Kibana), fail2ban для автоматической блокировки атак, OSSEC/Wazuh для HIDS, мониторинг дискового пространства через `df -h /var`. |
